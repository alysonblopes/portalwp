...
-- Schema Supabase (Postgres) — VERSÃO DE TESTE (sem login/portal)
--
-- Use este arquivo em um projeto Supabase novo e vazio, usado só para
-- testes, sem o Portal World Post por trás (sem portal_profiles, sem
-- autenticação). O app (erp/index.html) roda "aberto", direto com a
-- chave anon.
--
-- Diferenças em relação a schema.sql (versão integrada ao portal):
--   - Não depende de portal_profiles nem de auth.uid().
--   - RLS desligado (tabelas abertas para a chave anon) — não use em
--     produção com dados reais.
--   - RPCs sem checagem de permissão.
--
-- Quando o projeto sair da fase de testes e voltar a rodar junto do
-- Portal World Post, use schema.sql no lugar deste arquivo.
-- ══════════════════════════════════════════════════════════════════
...
-- ──────────────────────────────────────────────
-- 5. PRODUÇÃO (fábrica) — estrutura de produto (BOM)
-- ──────────────────────────────────────────────
...
-- ──────────────────────────────────────────────
-- 7. FUNÇÕES TRANSACIONAIS (RPC)
--    Garantem consistência entre pedido/produção e o kardex de estoque.
--    Sem checagem de permissão (fase de testes).
-- ──────────────────────────────────────────────
...
-- ──────────────────────────────────────────────
-- 8. PLANO DE CONTAS PADRÃO (seed opcional)
-- ──────────────────────────────────────────────
...
-- ══════════════════════════════════════════════════════════════════
-- IMPORTANTE: como o RLS está desligado, qualquer pessoa com a URL
-- e a chave anon deste projeto consegue ler e escrever nessas
-- tabelas. Use isso só em um projeto Supabase dedicado a testes,
-- nunca em um projeto com dados reais de clientes/fornecedores.
-- ══════════════════════════════════════════════════════════════════
